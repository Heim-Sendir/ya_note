...
def test_author_can_delete_note(author_client, slug_for_agrs):
    url = reverse('notes:delete', args=slug_for_agrs)
    response = author_client.post(url)
    assertRedirects(response, reverse('notes:success'))
    assert Note.objects.count() == 0


def test_other_user_can_delete(not_author_client, slug_for_agrs):
    url = reverse('notes:delete', args=slug_for_agrs)
